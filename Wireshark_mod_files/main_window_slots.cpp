...
void MainWindow::on_actionEditPacketComment_triggered()
{
    ...
    PacketCommentDialog *pc_dialog = new PacketCommentDialog(capture_file_.capFile()->current_frame->num, this, packet_list_->packetComment(), df_combo_box_->currentText(), proto_tree_->getKeypresses(), proto_tree_->getSystemcalls(), byte_view_tab_->getAscii(), byte_view_tab_->getHex());
    //pc_dialog.exec();
    pc_dialog->show();
}
...
void MainWindow::on_actionViewColorizePacketList_triggered(bool checked) {
    ...
    coloring_rules_dialog->colorize_button_clicked();
