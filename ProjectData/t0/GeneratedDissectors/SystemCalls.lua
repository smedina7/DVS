...
function SystemCalls_proto.dissector(buffer,pinfo,tree)
    ...
    if pinfo.abs_ts >= 1602016379.0 and pinfo.abs_ts <= 1602016381.0 then
       local subtree = tree:add(SystemCalls_proto,"SystemCalls Log")
       local mycomplientstr = tostring("wget google.com")

       subtree:add(timestamp_F,tostring("2020-10-06T20:32:59"))
       subtree:add(eventdata_F, mycomplientstr)
    end
    ...
    if pinfo.abs_ts >= 1602016386.0 and pinfo.abs_ts <= 1602016388.0 then
       local subtree = tree:add(SystemCalls_proto,"SystemCalls Log")
       local mycomplientstr = tostring("cat /home/kali/eceld-netsys/eceld/plugins/collectors/auditd/raw/1602016363_auditd.txt")
